...
def idrac_ss (esxi_host,idrac_credentials,jsd_ticket,jira_credentials,idrac_vendor):
    ...
    if idrac_response.status_code == 200 :
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            img.close()
            comment_body = f'Server vendor is {idrac_vendor}. Refer attachment ' + \
                    '[' + '^' + 'iDRAC_console.png' + ']' + ' for console status'
        ...
        comment_body = f'Server vendor is {idrac_vendor}. Unable to capture console screenshot. '
        jira_url = "https://servicedesk.xxx.com"
        comment_api_endpoint = f"/rest/api/2/issue/{jsd_ticket}/comment"
        url = jira_url + comment_api_endpoint
        # Construct payload and header
...
inputs = {
        'jsd_ticket' : 'xxx',
        'esxi_host' : "xxx-1-comp13.xxx.com",
        'jira_credentials' : 'xx=',
        'idrac_credentials' : 'xxM=',
        'idrac_vendor' : 'DELL'
    }
...
def handler (inputs):
    ...
    idrac_vendor = inputs['idrac_vendor']
    return idrac_ss (esxi_host,idrac_credentials,jsd_ticket,jira_credentials,idrac_vendor)
